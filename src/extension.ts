...
/**
 * Default CodeLLDB launch configuration written into new C++ workspaces.
 *
 * The `program` field is intentionally a placeholder so students see at a
 * glance that they need to point it at their compiled executable. For
 * typical EECS 280 projects, that's `${workspaceFolder}/main.exe` after
 * running `make main.exe`.
 */
const DEFAULT_LAUNCH_JSON = `{
  // Generated by the EECS 280 Setup extension. Update the "program" path
  // below to point at your compiled executable (e.g. "main.exe").
  "version": "0.2.0",
  "configurations": [
    {
      "type": "lldb",
      "request": "launch",
      "name": "Debug",
      "program": "\${workspaceFolder}/<your-program>.exe",
      "args": [],
      "cwd": "\${workspaceFolder}"
    }
  ]
 ...
 * file) and has no .vscode/launch.json, write a default CodeLLDB launch
 * config so the student can hit F5 to debug after pointing `program` at
 * their executable.
 *
 ...
 */
async function maybeCreateLaunchJson(): Promise<void> {
  ...
  for (const folder of folders) {
    ...
    try {
      ...
      fs.writeFileSync(launchPath, DEFAULT_LAUNCH_JSON, { flag: "wx" });
    } catch {
...
async function updateStatusBar(
  ...
  // launch.json already exists or the workspace has no C++ files.
  await maybeCreateLaunchJson();
